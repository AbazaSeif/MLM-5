...
class Layer_RecruitmentDocumentsController extends Zend_Controller_Action
{
	public function addAction()
	{
		$em = EntityManager::getInstance();
		$model = $em->create("Document");
		$form = new Recruitment_DocumentForm($model);
		if ($this->getHelper("FormAction")->simpleAction($form, $model, true)) {
			$recruitDocument = new \Model\RecruitDocument();
			$recruitDocument->recruit = $em->find("Recruit", $form->getValue("parent"));
			$recruitDocument->document = $model;
			$em->persist($recruitDocument);
