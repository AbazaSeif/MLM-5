...
class Layer_CustomerDocumentsController extends Zend_Controller_Action
{
	public function addAction()
	{
		$em = EntityManager::getInstance();
		$model = $em->create("Document");
		$form = new Customers_DocumentForm($model);
		if ($this->getHelper("FormAction")->simpleAction($form, $model, true)) {
			$customerDocument = new \Model\CustomerDocument();
			$customerDocument->customer = $em->find("Customer", $form->getValue("parent"));
			$customerDocument->document = $model;
			$em->persist($customerDocument);
